...
image_set = '../datasets/archive/Val/Val/Annotations'  # 需要转换的文件夹名称（文件夹内放xml标签文件）
imageset2 = '../datasets/archive/Val/Val/labels'  # 保存txt的文件夹
# 2.
# 换成你的类别 当前的顺序，就txt 0,1,2,3 四个类别
classes = ['person']  # 标注时的标签 注意顺序一定不要错。

# 3.
# # 转换文件夹的绝对路径
#data_dir = 'D:\program\yolo-v5\yolov5-master'
# 或者 读取当前路径
